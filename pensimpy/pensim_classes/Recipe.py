...
class Recipe:
    def __init__(self):
        factor = 20
        Fs_noise = np.random.randint(-factor, factor)
        Foil_noise = np.random.randint(-factor, factor)
        Fg_noise = np.random.randint(-factor, factor)
        pres_noise = np.random.randint(-factor, factor)
        discharge_noise = np.random.randint(-factor, factor)
        water_noise = np.random.randint(-factor, factor)
        ...
        pres_sp = [ele * (1 + pres_noise / 100) for ele in pres_sp]
        ...
        discharge_sp = [ele * (1 + discharge_noise / 100) for ele in discharge_sp]
        ...
        water_sp = [ele * (1 + water_noise / 100) for ele in water_sp]
